...
type BikesDatabase = Record<string, BikeModel[]>;

const BIKES: BikesDatabase = {
    Yamaha: [
...
const SmartGarageScreen: React.FC<Props> = ({ navigation }) => {
    ...
    const handleMakeSelect = (make: string): void => {
        ...
        setSelectedModel(BIKES[make][0]);
        setShowMakeModal(false);
    };

    const handleModelSelect = (model: BikeModel): void => {
                    ...
                    <View style={styles.card}>
                        ...
                        <View style={styles.visualizerContainer}>
                            {bars.map((barHeight, index) => (
                                <View
                                    key={index}
                                    style={[styles.bar, { height: `${barHeight}%` as any, backgroundColor: '#06B6D4', opacity: barHeight / 100 }]}
                                />
                            ))}
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <FlatList
                            ...
                            renderItem={({ item }: ListRenderItemInfo<BikeModel>) => (
...
const styles = StyleSheet.create({
    ...
    headerLogo: { width: 40, height: 40, borderRadius: 20, borderWidth: 1, borderColor: '#06B6D4' },
    sectionContainer: { marginBottom: 25 },
    sectionHeaderRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 10, gap: 10 },
    sectionTitle: { color: 'white', fontSize: 12, fontWeight: 'bold', letterSpacing: 1 },
    card: { backgroundColor: '#161925', borderRadius: 16, padding: 20, borderWidth: 1, borderColor: '#1F2937' },
    uploadContainer: { alignItems: 'center', marginBottom: 25, marginTop: 10 },
    uploadCircle: { width: 100, height: 100, borderRadius: 50, backgroundColor: '#1F2937', alignItems: 'center', justifyContent: 'center', zIndex: 2, overflow: 'hidden' },
    uploadOverlay: { position: 'absolute', bottom: 10, backgroundColor: 'rgba(0,0,0,0.6)', paddingHorizontal: 8, borderRadius: 4 },
    uploadText: { color: '#FFD700', fontSize: 8, fontWeight: 'bold' },
    photoRing: { position: 'absolute', top: -5, width: 110, height: 110, borderRadius: 55, borderWidth: 2, borderColor: '#FFD700', zIndex: 1, shadowColor: '#FFD700', shadowOpacity: 0.5, shadowRadius: 10 },
    formRow: { flexDirection: 'row', gap: 15 },
    inputGroupSmall: { flex: 1 },
    inputGroupLarge: { flex: 2 },
    inputGroupFull: { marginBottom: 15 },
    label: { color: '#6B7280', fontSize: 10, fontWeight: 'bold', marginBottom: 6 },
    dropdownInput: { backgroundColor: '#111827', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 12, borderRadius: 8 },
    textInputContainer: { backgroundColor: '#111827', padding: 4, borderRadius: 8 },
    textInput: { color: 'white', paddingHorizontal: 12, paddingVertical: 8, fontSize: 14, fontWeight: 'bold' },
    inputText: { color: 'white', fontSize: 14, fontWeight: 'bold' },
    topRightTag: { position: 'absolute', top: 20, right: 20, backgroundColor: '#06B6D410', borderWidth: 1, borderColor: '#06B6D4', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 4, zIndex: 5 },
    tagText: { color: '#06B6D4', fontSize: 10, fontWeight: 'bold' },
    brandIcon: { width: 16, height: 16, backgroundColor: '#9CA3AF', borderRadius: 2, marginRight: 10 },
    statsRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 10, paddingVertical: 20, borderTopWidth: 1, borderTopColor: '#1F2937' },
    statItem: { alignItems: 'center', flex: 1 },
    verticalDivider: { width: 1, backgroundColor: '#1F2937' },
    statLabel: { color: '#6B7280', fontSize: 10, marginBottom: 5 },
    statValue: { color: 'white', fontSize: 20, fontWeight: 'bold' },
    statUnit: { fontSize: 10, color: '#9CA3AF' },
    visualizerContainer: { height: 80, backgroundColor: '#111827', borderRadius: 8, flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between', padding: 10, position: 'relative', overflow: 'hidden' },
    bar: { width: 4, borderRadius: 2 },
    gridLines: { ...StyleSheet.absoluteFillObject, justifyContent: 'center' },
    gridLine: { height: 1, backgroundColor: '#1F2937', width: '100%', marginVertical: 10 },
    initButton: { marginTop: 10, marginBottom: 20, borderRadius: 12, overflow: 'hidden', shadowColor: '#FFD700', shadowOpacity: 0.3, shadowRadius: 10, elevation: 5 },
    gradientButton: { paddingVertical: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
    initButtonText: { color: 'black', fontWeight: 'bold', fontSize: 14, letterSpacing: 0.5 },
    modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.8)', justifyContent: 'center', padding: 20 },
    modalContent: { backgroundColor: '#161925', borderRadius: 16, padding: 20, maxHeight: '80%' },
    modalTitle: { color: '#FFD700', fontSize: 16, fontWeight: 'bold', marginBottom: 15, textAlign: 'center' },
    modalItem: { paddingVertical: 15, borderBottomWidth: 1, borderBottomColor: '#1F2937' },
    modalItemText: { color: 'white', fontSize: 16 },
    modalItemSub: { color: '#6B7280', fontSize: 12, marginTop: 2 },
    closeBtn: { marginTop: 20, alignItems: 'center', padding: 10, backgroundColor: '#1F2937', borderRadius: 8 },
    closeBtnText: { color: '#9CA3AF', fontWeight: 'bold' },
});
