...
const LobbyScreen = ({ navigation }) => {
    ...
    useEffect(() => {
        ...
        if (currentGroup && currentGroup.id) {
            rideUnsub = onSnapshot(doc(db, 'rides', currentGroup.id), docSnap => {
                if (docSnap.exists()) {
                    setRide(docSnap.data());
                }
            });
            membersUnsub = onSnapshot(collection(db, 'rides', currentGroup.id, 'members'), snapshot => {
                const memberList = [];
                snapshot.docs.forEach(docSnap => {
                    memberList.push({ id: docSnap.id, ...docSnap.data() });
                });
    ...
    const renderAvatar = (member, size = 45) => (
        <View style={[styles.regularAvatar, (member.id === ride?.hostId) && styles.hostAvatar, { width: size, height: size, borderRadius: size / 2, backgroundColor: '#374151', alignItems: 'center', justifyContent: 'center' }]}>
    ...
    // Handler function for backspace in join code input
    function handleInputBackspace(index, joinCode, setJoinCode, inputRefs) {
        if (index > 0) {
            const newCode = [...joinCode];
            newCode[index - 1] = '';
            setJoinCode(newCode);
