...
import { MainTabParamList, RootStackParamList, UserStats, RideData, AchievementsMap } from '../types';
...
type TabName = 'History' | 'Telemetry' | 'Achievements' | 'Routes';
...
const StatsScreen: React.FC<Props> = () => {
        ...
        <View>
            ...
            {history.length === 0 ? (
                ...
                history.map((ride, index) => (
                    <View key={ride.id || String(index)} style={styles.featuredCard}>
                        <LinearGradient colors={['#1F2937', '#111827']} style={styles.featuredCardHeader}>
                            ...
                            <View style={styles.datePill}>
                                <Text style={styles.dateText}>
                                    {(ride.timestamp as any)?.toDate ? (ride.timestamp as any).toDate().toLocaleDateString() : 'Recent'}
                                </Text>
                            </View>
                        </LinearGradient>
                    ...
                    return (
                        <View key={ach.id} style={[styles.badgeCard, !isUnlocked && { opacity: 0.3 }]}>
                            <View style={[styles.badgeIconBg, { backgroundColor: isUnlocked ? ach.color + '20' : '#1F2937' }]}>
                                <MaterialIcons
                                    name={isUnlocked ? ach.icon as any : 'lock'}
                                    size={24}
                                    color={isUnlocked ? ach.color : '#6B7280'}
                                />
                            </View>
    ...
    const TABS: TabName[] = ['History', 'Telemetry', 'Achievements', 'Routes'];
...
const styles = StyleSheet.create({
    ...
    gaugeArc: { width: 80, height: 80, borderRadius: 40, borderWidth: 4, borderColor: '#1F2937', borderTopColor: '#EF4444', borderLeftColor: '#EF4444', position: 'absolute', bottom: -30 },
    ...
    curveRating: { flexDirection: 'row', alignItems: 'center', gap: 2 },
