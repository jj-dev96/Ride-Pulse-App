...
import {
    ...
    TextInput, ActivityIndicator, Alert, Share, LayoutAnimation,
...
type Props = CompositeScreenProps<
    BottomTabScreenProps<MainTabParamList, 'CenterLogo'>,
    NativeStackScreenProps<RootStackParamList>
>;

type ActiveTab = 'JOIN' | 'HOSTING';
...
const LobbyScreen: React.FC<Props> = ({ navigation }) => {
    ...
    useEffect(() => {
        ...
        if (currentGroup?.id) {
            ...
            unsubMembers = GroupService.subscribeToMembers(currentGroup.id, (memberList: GroupMember[]) => {
                ...
                if (previousMemberCount.current !== 0) {
                    if (memberList.length > previousMemberCount.current) {
                        ...
                        if (newMember) showToast(`${newMember.name} joined the ride`);
                    } else if (memberList.length < previousMemberCount.current) {
                        const leftMember = members.find(m => !memberList.some((curr: GroupMember) => curr.id === m.id));
                        if (leftMember) showToast(`${leftMember.name} left the ride`);
                    }
                }
    ...
    const handleSwitchTab = (tab: ActiveTab): void => {
    ...
    const handleCreateLobby = async (): Promise<void> => {
        ...
        try {
            ...
            setCurrentGroup({ id, ...rideDetails, hostId: user.id } as GroupData);
            handleSwitchTab('HOSTING');
        } catch (error: unknown) {
            const err = error as { message?: string };
            Alert.alert("Error", "Failed to create lobby: " + err.message);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleJoinLobby = async (): Promise<void> => {
        ...
        try {
            ...
            setCurrentGroup(groupData as unknown as GroupData);
            handleSwitchTab('HOSTING');
        } catch (error: unknown) {
            const err = error as { message?: string };
    ...
    const onBarCodeScanned = ({ data }: { data: string }): void => {
        setScanning(false);
    ...
    const handleShare = async (): Promise<void> => {
        ...
        try {
            await Share.share({ message: `Join my RidePulse group! Use code: ${currentGroup.id}` });
        } catch (error: unknown) {
            const err = error as { message?: string };
            if (err.message) Alert.alert(err.message);
        }
    };
    ...
    return (
        <LinearGradient colors={['#0F111A', '#161925', '#111827']} style={styles.container}>
                            ...
                            <View style={styles.hostingContainer}>
                                {!currentGroup ? (
                                    ...
                                    <View style={styles.lobbyContainer}>
                                        ...
                                        <LinearGradient colors={['#1F2937', '#111827']} style={styles.accessCard}>
            ...
            {scanning && (
                <View style={StyleSheet.absoluteFillObject}>
                    ...
                    <View style={styles.scanOverlay}>
                        <Text style={styles.scanInst}>Point camera at QR code</Text>
...
const styles = StyleSheet.create({
    ...
    circularLogoContainer: { marginBottom: 15, width: 100, height: 100, borderRadius: 50, backgroundColor: '#FFD700', padding: 4, shadowColor: '#FFD700', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.5, shadowRadius: 10, elevation: 10 },
    ...
    tabsContainer: { flexDirection: 'row', backgroundColor: '#161925', borderRadius: 16, padding: 6, marginBottom: 20, borderWidth: 1, borderColor: '#374151' },
