    ...
    return (
        <View style={styles.tabBarContainer}>
            {state.routes.map((route, index) => {
                ...
                const getIcon = (name: string, color: string): React.ReactNode => {
                    switch (name) {
                ...
                if (route.name === 'CenterLogo') {
                    return (
                        <TouchableOpacity
                            ...
                            activeOpacity={0.8}
                            onPress={() => navigation.navigate('CenterLogo' as never)}
                        >
...
export default function AppNavigator(): React.ReactElement {
    ...
    const getInitialRoute = (): keyof RootStackParamList => {
        ...
        if (!user.profile?.profileCompleted && !user.skipProfileSetup) return 'ProfileSetup';
        return 'Main';
    };
    ...
    return (
        <NavigationContainer>
            ...
            >
                {user ? (
                    <>
                        {/* Both screens always registered; initialRouteName controls which shows first */}
